...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        SOV Related Inventory Messages
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION)
    {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION && chainActive.Height() >= nX16SOVFork-5)
        {
...
bool SendMessages(CNode* pto, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    const Consensus::Params& consensusParams = chainParams.GetConsensus();
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) { // NOTE: was "close to today" and 24h in SOV
        ...
        if (state.fSyncStarted && state.nHeadersSyncTimeout < std::numeric_limits<int64_t>::max()) {
            ...
            if (pindexBestHeader->GetBlockTime() <= GetAdjustedTime() - 6*60*60) { // was 24*60*60 in sov
