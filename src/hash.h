...
/* ----------- X16SOV Hash ------------------------------------------------ */
template<typename T1>
inline uint256 HashX16SOV(const T1 pbegin, const T1 pend, const uint256 PrevBlockHash)
{
//	static std::chrono::duration<double>[16];
    int hashSelection;

    sph_groestl512_context   ctx_groestl;    // 0
    sph_simd512_context      ctx_simd;       // 1
    sph_blake512_context     ctx_blake;      // 2
    sph_whirlpool_context    ctx_whirlpool;  // 3
    sph_sha512_context       ctx_sha512;     // 4
    sph_echo512_context      ctx_echo;       // 5
    sph_shavite512_context   ctx_shavite;    // 6
    sph_fugue512_context     ctx_fugue;      // 7
    sph_hamsi512_context     ctx_hamsi;      // 8
    sph_bmw512_context       ctx_bmw;        // 9
    sph_shabal512_context    ctx_shabal;     // A
    sph_cubehash512_context  ctx_cubehash;   // B
    sph_luffa512_context     ctx_luffa;      // C
    sph_skein512_context     ctx_skein;      // D
    sph_jh512_context        ctx_jh;         // E
    sph_keccak512_context    ctx_keccak;     // F
