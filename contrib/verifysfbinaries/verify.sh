...
###   This script attempts to download the signature file SHA256SUMS.asc from sov.org
...
RCSUBDIR="test/"
BASEDIR="https://sov.org/bin/"
VERSIONPREFIX="sov-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'sov-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SOV Core binary release signing key installed?"
   fi
